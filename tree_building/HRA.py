...
def hierarchicalCluster(distanceMatrix):
...
def processSingleCsv(filePath, outputDir):
    ...
    linkageMatrix = hierarchicalCluster(distanceDf.values)
    print(" - Constructed phylogenetic tree using hierarchical clustering method.")
    labels = distanceDf.index.tolist()
    outputImagePath = os.path.join(outputDir, f"{treeLabel}_HRA_Tree.png")
...
def HRA(generalFolderPath):
    inputDir = os.path.join(generalFolderPath, 'input')
    outputDir = os.path.join(generalFolderPath, 'result', 'HRA')
    ...
    if not distanceCsvFiles:
        ...
        if newickTrees:
            outputTreePath = os.path.join(outputDir, "All_HRA_Trees.nex")
